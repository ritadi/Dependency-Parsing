...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings):
        ...
        """
        # *** BEGIN YOUR CODE ***
        word_embedding = torch.tensor(word_embeddings,requires_grad = True)
        tag_embedding = he_initializer((self.config.n_tag_ids,self.config.embed_size))
        deprel_embedding = he_initializer((self.config.n_deprel_ids,self.config.embed_size))
        self.word_embeddings = word_embedding
        self.tag_embeddings = tag_embedding
        self.deprel_embeddings = deprel_embedding
        # *** END YOUR CODE ***
    ...
    def create_weights_biases(self):
        ...
        Wh_tensor = he_initializer((N * self.config.embed_size, self.config.hidden_size))
        Wo_tensor = he_initializer((self.config.hidden_size, self.config.n_classes))
        Bh_tensor = torch.zeros((self.config.hidden_size,),requires_grad=True)
        Bo_tensor = torch.zeros((self.config.n_classes,),requires_grad=True)
        self.W_h = Wh_tensor
        self.b_h = Bh_tensor
        self.W_o = Wo_tensor
        self.b_o = Bo_tensor
    ...
    def embedding_lookup(self, id_batch, n_ids, embedding_matrix):
        ...
        one_hot_vector = one_hot_float(id_batch,n_ids)
        relevant_embeddings = torch.matmul(one_hot_vector, embedding_matrix)

        embedded_batch = torch.reshape(relevant_embeddings,(B,N * self.config.embed_size))
        #*** END YOUR CODE ***#
    ...
    def get_concat_embeddings(self, word_id_batch, tag_id_batch,
        ...
        """
        # *** BEGIN YOUR CODE ***
        embedded_batch_word = self.embedding_lookup(word_id_batch,self.config.n_word_ids,self.word_embeddings)
        embedded_batch_tag = self.embedding_lookup(tag_id_batch,self.config.n_tag_ids,self.tag_embeddings)
        embedded_batch_deprel = self.embedding_lookup(deprel_id_batch,self.config.n_deprel_ids,self.deprel_embeddings)
        x = torch.cat((embedded_batch_word, embedded_batch_tag, embedded_batch_deprel),1)
        # *** END YOUR CODE ***
        return x
    ...
    def forward(self, word_id_batch, tag_id_batch, deprel_id_batch):
        ...
        h_drop = F.dropout(h, self.config.dropout,False)
    ...
    def add_optimizer(self):
        ...
        """
        # *** BEGIN YOUR CODE ***
        parameters = [self.word_embeddings, self.tag_embeddings, self.deprel_embeddings, self.W_h, self.W_o, self.b_h, self.b_o]
        self.optimizer = torch.optim.Adam(parameters,self.config.lr)
        # *** END YOUR CODE ***
