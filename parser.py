...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id not in [self.left_arc_id, self.right_arc_id, self.shift_id]:
        ...
        elif len(self.stack) < 2 and transition_id != self.shift_id:
            raise ValueError('stack less than two elements')
        else:
            if transition_id == self.shift_id:
                if self.next < len(self.sentence):
                    ...
                    raise ValueError('reach the end of the sentence!')
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        for tup in self.arcs:
            if tup[0] == sentence_idx:
                dependants.append(tup[1])
        dependants.sort()
        if n is None:
            deps = dependants
        elif n == 0:
            deps = []
        elif len(dependants) < n :
            deps = dependants
        else:
            deps = dependants[:n]
...
#        # *** BEGIN YOUR CODE ***
        ...
        if len(self.stack) == 1 and self.next < len(self.sentence):
            ...
            cur_node_idx = self.stack[-1]
            prev_node_idx = self.stack[-2]

            cur_node = graph.nodes[cur_node_idx]
            prev_node = graph.nodes[prev_node_idx]

            left_dep = list(get_left_deps(cur_node))
            right_dep = list(get_right_deps(cur_node))

            prev_left_dep = list(get_left_deps(prev_node))
            prev_right_dep = list(get_right_deps(prev_node))
            for dep in right_dep:
                if dep >= self.next:
                    right_arc = False
            for dep in prev_right_dep:
                if dep >= self.next:
                    left_arc = False

            ##left arc
            if graph.nodes[prev_node_idx]['head'] == cur_node_idx and left_arc:
                transition_id = self.left_arc_id
                deprel = graph.nodes[prev_node_idx]['rel']
            elif graph.nodes[cur_node_idx]['head'] == prev_node_idx: ##prev->cur
                if right_arc or (len(self.stack) == 2 and self.next == len(self.sentence)):
                    transition_id = self.right_arc_id
                    deprel = graph.nodes[cur_node_idx]['rel']
                else:
                    transition_id = self.shift_id
                    deprel = None
            else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for i in range(len(minibatch)-1,-1,-1):
            ...
            deprel = td_pairs[i][1]
            try:
                for j in range(0, len(partial_parses)):
                    if partial_parses[j] == minibatch[i]:
                        partial_parses[j].parse_step(t_id, deprel)
                        if partial_parses[j].complete:
                            unfinished_parses.pop(i)
            except ValueError:
                unfinished_parses.pop(i)
    for parse in partial_parses:
        result.append(parse.arcs)
    arcs = result
